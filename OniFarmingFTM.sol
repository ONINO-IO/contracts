...
contract Staking is Initializable, OwnableUpgradeable {
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > lastRewardBlockTime && lpSupply != 0) {
            uint256 Seconds = block.timestamp.sub(lastRewardBlockTime);
            uint256 SecondReward = Seconds.mul(rewardPerSecond);
            accRewardPerShare_ = accRewardPerShare_.add(
                SecondReward.mul(ACC_REWARD_PRECISION) / lpSupply
            );
        }
        pending = int256(
    ...
    function updatePool() public {
        if (block.timestamp > lastRewardBlockTime) {
            ...
            lastRewardBlockTime = block.timestamp;
            emit LogUpdatePool(lastRewardBlockTime, lpSupply, accRewardPerShare);
        }
    }
