...
contract OniStake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event NewStartAndEndBlocks(uint256 startBlockTime, uint256 endBlockTime);
    event NewRewardPerSecond(uint256 rewardPerSecond);
    event RewardsStop(uint256 blocktimestamp);
    event Withdraw(address indexed user, uint256 amount);

    constructor() public {
        stakedToken = IBEP20(0x667c856f1A624BAeFE89fC4909C8701296C86c98);
        rewardPerSecond = 10**18 / 100; // 0.01 token per block
        startBlockTime = block.timestamp;
        bonusEndBlockTime = block.timestamp.add(31556926); // One year
    ...
    function stopReward() external onlyOwner {
        bonusEndBlockTime = block.timestamp;
    }
     ...
     * @param _startBlockTime: the new start block
     * @param _bonusEndBlockTime: the new end block
     */
    function updateStartAndEndBlocks(uint256 _startBlockTime, uint256 _bonusEndBlockTime) external onlyOwner {
        require(_startBlockTime < _bonusEndBlockTime, "New startBlockTime must be lower than new endBlock");
        require(block.timestamp < _startBlockTime, "New startBlockTime must be higher than current block");
        ...
        emit NewStartAndEndBlocks(_startBlockTime, _bonusEndBlockTime);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardBlockTime && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlockTime, block.timestamp);
            uint256 rewardAmount = multiplier.mul(rewardPerSecond);
